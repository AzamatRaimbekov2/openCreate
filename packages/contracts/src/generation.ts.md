...
- Responsibilities: validate creation input (prompt 2–2000 chars, known aspect ratio, optional duration 1–15s, optional `inputImage` as data URI ≤14MB) and the full DTO (status/progress/mediaUrls/errorMessage/timestamps).
...
## Dependencies
- Imports / depends on: `zod`, `./catalog` (`aspectRatioSchema`).
