...
- Responsibilities: construct `buildApp(deps)` with deterministic test config and a fresh `createDb(':memory:').db` per call (test isolation).
...
## Dependencies
- Imports / depends on: `src/app` (`buildApp`), `src/db/client` (`createDb`).
...
- (pending) feat(api): drizzle schema + sqlite bootstrap DDL — in-memory db injected
