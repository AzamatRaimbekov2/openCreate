...
- Responsibilities: parse + default env vars; normalize empty Google OAuth creds to `null`; hydrate `process.env` from the nearest `.env` file on the default boot path.
- Public API / exports: `AppConfig` (type), `LogLevel` (type), `loadConfig(env?): AppConfig`, `loadEnvFromFile(path?): void`.
