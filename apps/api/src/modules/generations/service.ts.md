...
- Public API / exports / props / endpoints:
  ...
  - `create(userId, input, reqLog?)` → `{ dto: Generation, created: boolean }` — `created: true` = the row was created by THIS call (route → 201); `false` = an existing row was returned. NOTE it is no longer a proxy for "the image is finished": an image on an asynchronous backend is created (`true`) and still `processing`, so read `dto.status`, never `created`. Throws `ValidationError` (400), `InsufficientCreditsError` (402, from ledger), `RunwareError` (502). `reqLog` (routes pass `req.log`) stamps every money-path line of this call with the reqId.
...
flowchart TD
  ...
  R -- image --> IP[ImageProvider chain: kie/Seedream → runware] --> IK{kind}
  IK -- done --> S[storage.saveFromUrl] --> GTX[guarded flip → succeeded → 201]
  IK -- pending --> PUB[publish job id → 202, settles on a later poll]
...
- The VIDEO path now calls a `VideoProvider` from a `videoProviders` registry instead of `runware.*` directly. Provider is resolved from the catalog model's `provider` at submit (`providerId`, default `runware`) and from the row's persisted `provider` at poll (durable — a job always polls the backend it submitted to). The IMAGE path went the same way on 2026-08-19: it now calls an `ImageProvider` (a failover chain, kie/Seedream first, Runware behind it) whose `submit` returns EITHER `{kind:'done'}` — settled in-request, byte-for-byte the old behaviour — or `{kind:'pending'}`, which parks the row and reuses the video poll path. The poll seam is chosen by the ROW's type, so an in-flight job always polls the family it submitted to.
