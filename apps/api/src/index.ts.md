...
- `host: '::'` and not `0.0.0.0`: dual-stack, so the Vite dev proxy and
  containers still reach it over IPv4, AND a managed platform can reach it over
  the IPv6 internal network it health-checks on. An IPv4-only listener there is
  a container that boots, logs happily and is never reachable — a 502 with
  nothing in the logs to explain it (railway-deployment R2).
