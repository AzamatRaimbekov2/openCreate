...
- Responsibilities: `loadConfig()` → `createDb(config.databasePath)` (runs idempotent DDL bootstrap) → `buildApp({ config, db })` → `listen({ port, host: '0.0.0.0' })`.
...
## Dependencies
- Imports / depends on: `./app` (`buildApp`), `./config` (`loadConfig`), `./db/client` (`createDb`).
