...
animated `AsciiSphere` canvas behind centered content — big mono wordmark,
quiet kicker, whisper-weight mono display headline (weight 400, ONE portal
accent word), the approved claims line in mist, and TWO specimen-pill CTAs
(green "start creating" + amber "see pricing"). The text IS the hero.

## What it does (for an AI reader)
- Responsibilities: center the brand plate (`openCreate·` — the landing
  masthead carries no wordmark, this is it), kicker (`landing.kicker`), h1
...
flowchart LR
  ...
  Sphere[AsciiSphere absolute inset-0 pointer-events-none] --> Hero
