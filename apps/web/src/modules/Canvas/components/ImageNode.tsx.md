...
- **While a branch is running, the ghost pill IS the way out of it.** It used to be merely disabled, and `cancel()` — written, exported and unit-tested — was called by nothing: once a queue started, every credit in its plan was spent whatever the user did. `isBranchRunning` now swaps the pill to `canvas.runBranch.stop` calling `branch.cancel()`. The confirm dialog protects the FIRST charge; this protects the rest.
- **An upload parent disables Generate, and now says so.** `edgeRules` allows `upload → image` and the wire draws, but `buildRunInput` refuses it (a stored file has no generation id to cite until the operations phase). Disabling silently is the defect the refused-wire toast was built to fix — a rule working as designed, indistinguishable from a broken feature. `blockedByUpload` renders the amber `canvas.node.uploadInputHint` beside the character/model hint. The condition is narrow (`kind === 'upload'`) so it never steals the explanation from a missing prompt or an uncast character.
- Branch failures render through `branchErrorMessageKey`, not the shared `errorCodeMessageKey`: `timeout` and `not_runnable` are the queue's own codes and would each degrade to the generic "something went wrong".
- **Two pills, phase 3b** (spec §3): `Generate` runs THIS node, the ghost `Run branch` runs everything it depends on. The plan is built at CLICK time (`branch.buildPlan(id, data.models)` → local `plan` state → `RunBranchDialog`), never per render: it reads the whole graph plus the live status cache, and a render-time plan would be stale by the time the user answered anyway. Confirm closes the dialog FIRST and then runs — a modal hanging over a running board hides the progress it caused.
- **Both pills disable while a queue owns this node.** `useIsBranchBusy(id)` gates Generate (a manual submit mid-branch is the double-charge the confirm dialog exists to prevent) and `useIsBranchRunning()` gates the pill (two overlapping queues would interleave submits and make a confirmed total meaningless). The node subscribes to those two NARROW selectors rather than the whole run state, so a branch advancing does not re-render every card on the board.
- **`useBranchNodeError(id)`** renders a quiet amber line for a branch failure that never produced a generation row — `insufficient_credits` is the common one, and with no failed run there is no error card for it to land in. Suppressed when the shown run itself failed, because that card already tells the story with the refund chip.
