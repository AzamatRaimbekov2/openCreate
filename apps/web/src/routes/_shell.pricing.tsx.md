...
verified price comparison + live per-model credit table + the
200-free-credits signup CTA for visitors.

## What it does (for an AI reader)
- Responsibilities: compose `PriceTable` (modules/Landing), the catalog query
  (`useCatalog` from modules/Generator) with all 4 UI states around
  `ModelCreditTable`, and a session-aware signup CTA. Composition only.
