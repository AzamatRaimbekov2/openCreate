// apps/web/src/shared/libs/mentions.ts
...
//
// LIVES IN shared/, not in a module, because BOTH the Generator (one-off images
// and clips) and CinemaStudio (a film's shots) tag characters, and modules may
// not import each other. Two copies of this invariant would be two chances for it
// to drift — and a drifted mention protocol means the user tags a character, pays,
// and receives a stranger. It is protocol plumbing over strings, not domain logic:
// the syntax it speaks is defined in contracts, and it holds no state.
