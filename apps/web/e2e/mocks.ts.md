...
- Responsibilities: hold per-test backend state (`created`, `prompt`, `polls`, `createdAt`), answer the exact endpoints the SPA calls, and script the async video lifecycle: POST → 202 processing/0% → first GET :id → processing/40% → second GET :id → succeeded with `/media/gen-e2e-1.mp4`.
- Public API / exports:
...
- The generation's `createdAt` is stamped fresh (`new Date().toISOString()`) at POST time, NOT fixed: `useLiveGeneration` enforces a 20-minute polling budget (`GENERATION_STALL_MS`) from `createdAt` — the original fixed `2026-07-06T10:00Z` timestamp put every later run past the budget, so the SPA (correctly) stopped polling and rendered the stalled "taking longer" card instead of the succeeded `<video>` (final-gate e2e fix).
