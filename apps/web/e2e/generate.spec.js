...
test('landing → create → video generation lifecycle with balance decrease', async ({ page }) => {
    ...
    await page.goto('/');
    await expect(page.getByRole('heading', { name: 'Create images & video. Pay pennies, not plans.' })).toBeVisible();
    ...
    await expect(page.getByRole('button', { name: 'Credits balance' })).toHaveText(new RegExp(`${START_BALANCE}`));
    ...
    await expect(page.getByRole('button', { name: 'Credits balance' })).toHaveText(new RegExp(`${START_BALANCE - COST}`));
});
