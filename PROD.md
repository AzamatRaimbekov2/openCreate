...
| `KIE_API_KEY` | **yes for images** | – | kie.ai key. Every catalogue image model runs on Seedream 5 there, with Runware as the failover link — so WITHOUT this key images still work, but every one of them falls back to Runware and is sold BELOW COST (7 credits charged against up to $0.138 of Nano Banana). It also adds `tempfile.aiquickdraw.com` to the SSRF allowlist, which is where kie serves finished assets from. |
| `MEDIA_PUBLIC_BASE_URL` | no | falls back to `BETTER_AUTH_URL` | Where OUR `/media/*` is reachable from the public internet. Only reference-image generations need it: kie.ai fetches references by URL and rejects data URIs. A correct single-origin deploy needs nothing here. On localhost it cannot work by definition (kie cannot reach your laptop) — such jobs refuse at kie and run on Runware, which takes the bytes inline. |
