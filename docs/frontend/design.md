...
| `/cinema/$filmId` | CinemaStudio editor: `FilmEditorHeader` (back chevron, title, aspect chip, film `Menu` → rename/delete-confirm), the horizontal `Timeline` strip (abyss well of `ShotThumb`s with live clip status, amber-ring selection, move/delete + add-shot/title/storyboard), and a two-column deck of white/10 panels — left: `PreviewPlayer` (sequential DOM player + "approximation" caveat) · `RenderBar` (green download / calm retry) · `AudioTracks`; right: `ShotInspector` (prompt + preset `Select`s + video-model + duration/transition/title + green Generate). Preset pills reuse the amber selection language; status is text + triad glow, never color-only. |
