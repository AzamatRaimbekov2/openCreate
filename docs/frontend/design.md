...
|---|---|---|
| Auth | `AuthForm` (`/login`) | No card — printed on the void: mono 30px h1 over a white/10 hairline, steel `Input` fields + green pill submit; login↔register switch as a portal link; zod errors per field (`role="alert"`); server banner = steel block + glow-red left rule; Google button = amber ghost, behind `VITE_GOOGLE_AUTH=1`. |
| Auth | `AuthManifesto` (`/login` left) | `bg-abyss` panel: mono wordmark→`/` (portal dot), lowercase kicker, mono 400 30px quote (`auth.manifesto.*`), the three `landing.claims.*` as white/10 ledger rows (claims copy has ONE source of truth). |
| Credits | `BalanceChip` (header) | Mono caption chip at 40px: `rounded-full border-white/10 bg-white/5`, glow-AMBER bolt + numeral (`font-medium` — the 500 ceiling), hover → ridge; the bolt is a decorative `currentColor` SVG — NEVER an OS emoji; loading = pill `Skeleton`; failure = ↻ icon-button; signed-out hidden. Opens history modal. |
...
| Generator | `GeneratorPanel` (`/create`) | The "commission sheet": unfilled `rounded-lg border-white/10` frame on the void (steel inputs inside keep the elevation ladder readable), mono caption head, ordered `SheetField` rows (`divide-white/10`), white mono `CostLabel` + green Generate against the closing hairline. Catalog 4 states. Submit failures inline via `SubmitErrorBanner`. |
...
| Generator | `ModelPicker` | `aria-pressed` cards: `rounded-lg` white/10 hairline, `font-medium text-white` name, honest provider label + price hint; SELECTED = amber tint (`border-glow-amber/60 bg-specimen-amber/20`, amber price hint) — the reference's "model picker highlights = amber". |
