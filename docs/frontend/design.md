...
| Generator | `SubmitErrorBanner` | `role="alert"` steel block + glow-red left rule; maps `insufficient_credits` (+ portal `/pricing` link) and `content_blocked` to dedicated copy. |
| Generator | `ModelPicker` | `aria-pressed` STEEL tiles (`rounded-lg bg-steel` + white/10 hairline — "cards on #1d293d"), `font-medium text-white` name, honest provider label + price hint; SELECTED = the AMBER RING (`border-glow-amber/60` + amber price hint; no tint wash — the surface stays steel) per the reference's "model picker highlights = amber"; unselected hover steps to ridge. |
...
(Tasks 16-17) — §9's "no raw server text" otherwise stands; `auth.manifesto.*`
(and the retired `landing.showcase.figure`/`items.*`) keys remain in BOTH locale
files though no longer rendered — the keys-intact rule outlives the components.
