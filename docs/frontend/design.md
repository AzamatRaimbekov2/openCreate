...
| `/cinema/$filmId` | CinemaStudio editor, v7 — a real NLE WORKBENCH: ONE viewport-height column (`h-[calc(100svh-76px)]`, no page scroll). STAGE up top scrolls inside itself: `FilmEditorHeader` (back chevron, `text-lg` title, aspect chip, ⋯ `Menu` leading with «Собрать mp4» — export state owned by FilmEditor; hidden via `isAvailable` while a render runs — then rename/delete-confirm), the TRANSIENT `RenderBar` status strip (idle = nothing; progress → green Download → calm retry), `PreviewPlayer` (canvas capped `max-h-[42svh]`, "approximation" caveat). WORKBENCH pinned at the bottom: the composer dock (v6.2 — glass auto-grow prompt with the TOP-edge resize grip, label-less model chip → `ModelPickerModal`, stepped duration slider, speaker/cast/voice/expand toggles, Save + green Generate; slim hint row when nothing selected) above the TRACKS panel (`Timeline` v7): a RULER (ticks/s, labels /5s), the VIDEO lane (`ShotThumb` slots as wide as their duration — `--shot-w` ∝ durationMs on one `PX_PER_SEC=24` scale), and the AUDIO lane directly beneath (music beds as amber bars to the film's end, voiceover chips at their startMs in green, hover 🗑) — all three in ONE horizontal scroll so they cannot drift. The «+» dialog adds shot · title · storyboard · music · voiceover (audio rows switch to a mini-form; one charged Generate). The «Звук» card is retired. Status is text + triad glow, never color-only. |
