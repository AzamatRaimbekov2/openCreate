...
| Canvas | `NodePalette` | A 96px rail of kind buttons that are BOTH drag sources and click targets — drag-and-drop is not keyboard-reachable, so the click path is the accessible one, not a convenience. Glyphs are `aria-hidden`; the localized label is the accessible name. It lists only kinds that can actually run today (character / upscale / remove-bg exist in the contract and the edge rules already, but a button for a node that does nothing is a promise the product does not keep). |
