...
## [2026-08-26] fix | Холст: очередь всегда заканчивается, и её можно остановить

- Найдено аудитом холста перед его расширением до сборки фильмов. Веб-сюита была
  зелёной (128 файлов / 1068 тестов) и НИ ОДИН из трёх дефектов не ловила — все
  три живут в местах, куда тесты не смотрели.
- **Очередь залипала в `running` навсегда.** У цикла `useRunBranch.run` было три
  выхода голым `return`: уход с доски, исчезнувший узел и исчерпание 20-минутного
  бюджета опроса. `useBranchRunStore` — модульный синглтон, и маршрут при выходе
  сбрасывает store ДОКУМЕНТА, а не его. Значит `useIsBranchRunning()` оставался
  истиной до перезагрузки вкладки, и «Запустить ветку» была мертва на каждом узле
  каждой доски. Теперь каждый выход проходит через `settle()`, и он же несёт
  вторую половину — guard `activeRun !== thisRun`: прогон, которого уже сменил
  следующий, не имеет права затирать чужое состояние на выходе.
- **Почему это не поймал существующий тест.** Тест «stops when the board changes
  under it» проверял `expect(posts).toHaveLength(1)` — что ТРАТА прекратилась.
  Что доска освободилась, не проверял никто. Утверждение дописано в тот же тест.
- **Уход с доски и таймаут кончаются по-разному, и это осознанно.** Уход → `idle`
  (никто на ту доску не смотрит, а ошибка, приколотая к покинутому холсту,
  встретила бы пользователя на СЛЕДУЮЩЕМ). Таймаут → `failed` с кодом `timeout`
  (пользователь на месте, и очередь, замолчавшая без объяснения, неотличима от
  завершившейся). `pollToTerminal` поэтому возвращает `'cancelled' | 'timeout'`
  вместо общего `null` — различить их постфактум вызывающий не может.
- **`timeout` и `not_runnable` — не API-коды.** Через общий `errorCodeMessageKey`
  каждый деградировал до «Что-то пошло не так», что для таймаута неправда дважды:
  ничего не сломалось, и генерация продолжается. Добавлен `branchErrorMessageKey`
  — коды очереди сначала, потом общий маппинг (сам тотальный). Заодно исправлена
  копия для `not_runnable`, которая была неверной с самого начала.
- **Запущенную ветку было нельзя остановить.** `cancel()` был написан, экспортирован
  и покрыт юнит-тестом — и не вызывался ни одним компонентом; «Отмена» в диалоге
  закрывает подтверждение ДО запуска. Пилюля «Запустить ветку» во время прогона
  больше не просто отключена, а становится «Остановить ветку». Диалог защищает
  ПЕРВОЕ списание, эта кнопка — все остальные.
- **Провод от «Загрузки» отказывал молча.** `edgeRules` разрешает `upload → image`,
  провод рисуется, а `buildRunInput` его отвергает (у файла нет id генерации,
  цитировать нечего до фазы операций). Generate гас без единого слова — ровно тот
  дефект, ради которого 2026-08-03 появился тост об отказанном проводе. Теперь
  карточка говорит причину и выход янтарной строкой, рядом с уже существующей
  подсказкой про модель и персонажа. Условие узкое (`kind === 'upload'`), чтобы
  не перехватывать объяснение у пустого промпта или невыбранного персонажа.
- **Не сделано и почему.** Сделать загрузку ЦИТИРУЕМОЙ — это фича, а не фикс:
  нужен приём «staged generation» (он уже есть в фильмах, `film.ts:9`), а значит
  контракт, проверка владения на сервере и ADR. Отдельно не покрыт тестом guard
  `activeRun !== thisRun` — сценарий требует двух перекрывающихся прогонов, и
  честный тест на него выходил хрупким; оставлен комментарий в коде.
- Проверено: Canvas 118/118, полная веб-сюита 128 файлов / 1071 тест, `eslint
  src/modules/Canvas` чист, `tsc --noEmit` чист, обе локали парсятся.

## [2026-08-20] feat | Shorts Studio, фаза 1: пакет фильмов из шаблона — за ноль кредитов

- Серверная половина [[shorts-studio]]: полка `shorts` в `templateCategorySchema`,
  колонка `film.batch_id`, `POST /api/films/from-template/batch` и фильтр
  `GET /api/films?batchId=`. Раннер, диалог со сметой и сами шаблоны — не здесь.
- **Пакет ничего не стоит, и это проверено тестом, а не намерением.** Закон
  [[template-catalog]] («применить шаблон — бесплатно») распространён на N фильмов
  дословно: ни один модуль на этом пути не импортирует леджер, а тест меряет баланс
  до и после пакета из трёх строк на ПРЕМИУМ-тарифе. Кредиты уходят потом, по битам.
- **Одна плохая строка отменяет весь запрос — до первой записи.** Подстановка и
  проверка ручек вынесены в `planFilm`, который не трогает базу; `instantiateBatch`
  планирует ВСЕ строки и только затем пишет. Половина пакета хуже, чем ничего:
  четыре фильма из пяти выглядят точно так же, как законченные.
- **Все N фильмов — одна транзакция.** better-sqlite3 синхронный, между вставками
  нечего ждать, так что пакет ложится целиком или не ложится вовсе.
- **Ловушка индекса, из-за которой стоит читать `ddl.ts` перед правкой.**
  `CREATE TABLE IF NOT EXISTS` на существующем томе — пустышка, значит `batch_id`
  там появляется только после `ALTER TABLE`, а он идёт ПОСЛЕ DDL. Индекс, написанный
  рядом с таблицей, прошёл бы все тесты на чистой базе и уронил бы каждый живой
  деплой с `no such column: batch_id`. Он вынесен в отдельную константу, а
  `db-ddl.test.ts` теперь поднимает файл со СТАРОЙ схемой, чтобы это не вернулось.
- **`batchId` — происхождение, а не заявка клиента.** Поля для него нет ни в одном
  входном контракте: сервер чеканит `randomUUID()` внутри и никакой параметр не
  принимает. Та же дисциплина, что у `template_id`.
- Потолок — 20 строк, и это число лимитера сабмитов (20/мин), а не круглая цифра.
