...
## Implementation note — фаза 2 (фронтенд), 2026-07-30

Фазы 1–2 в проде. Фронт: `apps/web/src/modules/Creator` + `routes/_shell.creator.tsx`
+ пункт навигации `nav.creator` («Агент») сразу после `/create`. Подробности —
`apps/web/FEATURE.md` § openCreator.

Что уточнилось относительно решений ADR при реализации UI:

- **D2 (бюджетный гейт) на клиенте — это функция над всей перепиской, а не свойство
  карточки.** `model/planState.ts` классифицирует каждый план как `live` / `answered` /
  `stale`: второй план отменяет первый, и карточка, судящая по своему содержимому, могла
  бы подтвердить сумму, которую агент уже заменил. Заодно выяснилось, что фронт НЕ может
  отличить «подтверждено» от «устарело» — в сообщении плана нет поля исхода, — поэтому в
  состоянии `answered` он не утверждает ничего, а объяснение берут сообщения ниже. Если
  такое различие когда-нибудь понадобится в UI, его надо добавить в контракт (например
  `kind:'plan'` c полем `outcome`), а не выводить из текста.
- **Опрос идёт и в `awaiting_confirm`, не только в `running`.** Из ворот выходят без
  участия вкладки: подтверждение из второй вкладки, новое сообщение (сбрасывает
  `confirmed`), реапер. Иначе живая кнопка «Подтвердить» висит над отменённым планом.
- **D4 (переписка = состояние) держится, но текст ошибки в переписке оказался
  непереведённым.** Сервер кладёт в транскрипт одну из трёх фиксированных АНГЛИЙСКИХ
  фраз (`brain.ts`: две у `CreatorUnavailableError`, `service.ts`: `'The agent turn
  failed'`), а продукт русскоязычный. Клиент отображает это замкнутое множество в i18n
  (`model/agentCopy.ts`, только точное совпадение) — но джойн по ТЕКСТУ хрупок:
  правильное долгосрочное решение — код ошибки в контенте сообщения, тогда маппинг
  станет таким же, как `shared/libs/errorCopy.ts` для транспортных кодов.
- Выбор диалога живёт в локальном состоянии (экран сам открывает самый свежий), а не в
  URL: ни один экран приложения пока не использует search-параметры. Следствие — выбор
  не шарится ссылкой; апгрейд — `?session=`.
