...
- [[photo-to-3d-studio]] - **Proposed ADR** (2026-07-11, pending approval): Studio3D — photo → 3D model → presentable artifact. `model3d` becomes a fourth `generation.type` on the UNCHANGED async lifecycle + money path; **Runware's existing `3dInference` task type** is the provider (TRELLIS.2 / Hunyuan3D / Tripo → GLB), so no new integration. **Self-hosting 3D is rejected on arithmetic** — hosted TRELLIS.2 is $0.0256/model vs $7–12/day for a GPU (break-even ~300–470/day) and it wants the whole 24 GB the 4090 already gives Wan 2.2. Presentation = a portable **scene preset** (one JSON, N renderers) + a three.js viewer + a `model_render` job (no ledger, `film_render` precedent) whose clip drops into the CinemaStudio timeline. Open fork: which renderer makes the video.
- [[modular-3d-assets]] - **Proposed ADR** (2026-07-17, pending approval): Modular 3D Assets — AssetHub-style concept image → Claude-suggested named parts (free) → per-part extraction images (charged, ref-capable model) → per-part `model3d` meshes (existing Studio3D path) → client-side assembly + merged GLB export. New `asset3d`/`asset3d_part` aggregate CITES generations; zero new money code; Runware `3dInference` stays the only built mesh channel (D5 arithmetic unchanged).
- [[google-oauth]] - **Accepted ADR** (2026-07-23): Complete Google OAuth (better-auth provider). Enablement is RUNTIME, not a build flag: new public `GET /api/auth/config` → `{ googleEnabled }` (contract `authConfigSchema`) is the single source of truth the SPA reads via `useAuthConfig` to render the Google button, killing the old `VITE_GOOGLE_AUTH` drift. Dev cross-origin: set `BETTER_AUTH_URL=http://localhost:5173` so the OAuth callback routes through the Vite proxy. Creds are owner-supplied in `.env`.
- [[mcp-server]] - **Accepted ADR** (2026-07-22): `@opencreate/mcp` — a thin MCP server wrapping the openCreate REST API so a Claude user can create content and whole film "projects" from Claude Desktop/Code (the Higgsfield-MCP analogue). Local stdio now, transport-agnostic core for a remote HTTP + OAuth "Connect button" in Phase 2; email+password-in-env auth (zero backend change); all ~35 endpoints as one-tool-per-action, reusing `@opencreate/contracts` zod schemas. Spec: `docs/superpowers/specs/2026-07-22-opencreate-mcp-design.md`.
- [[canvas-mode]] - **Accepted ADR** (2026-07-30): Canvas Mode — Freepik/Flora-style node-graph canvas (`/canvas`) for chained generations (character → image → remix → video → upscale) OVER the unchanged generation lifecycle. Canvas aggregate CITES generations (film precedent); one contract delta (`inputGenerationId`); upscale/remove-bg as catalog pseudo-models; engine `@xyflow/react`. Spec: `docs/superpowers/specs/2026-07-29-canvas-mode-design.md`.
- [[canvas-prompt-node]] - **Accepted ADR** (2026-08-02): the canvas `prompt` node — a card holding shared prompt text that several image/video nodes read through a wire (retires `canvas-mode`'s "Out of scope: node templates"). A node KIND, not a new entity: one enum value in `canvasNodeKindSchema`, the text in the existing `config.prompt`, zero API logic. The wire MERGES (`template ⏎ own`, template first) rather than replaces; ONE `composeNodePrompt` answers the run path, the branch plan and the card, so Generate can never disagree with what gets charged; a third edge slot, cap 1. Cross-board reuse (the `/styles` shape) stays out of scope.
- [[railway-deployment]] - **Accepted ADR** (2026-08-02): production deployment on **Railway** — the existing Dockerfile as the deploy unit, **one replica with deploy overlap disabled** (SQLite is single-writer, so a rolling deploy would put two processes on one database file), volume at `/app/data` for SQLite only, **media moved to Cloudflare R2 from day one** (`StorageProvider`'s `dir`/`localPath` replaced by `serve`/`materialize`/`publishLocalFile`; `/media/*` becomes a 302 to a public R2 domain, which lowers no bar — it is already unauthenticated by design), and **GitHub Actions as the single quality gate** deploying by token rather than the platform's own auto-deploy. Spec: [[infrastructure-railway]].
- [[infrastructure-railway]] - Execution spec for [[railway-deployment]]: topology + request-path diagrams, full production env matrix, `railway.json`, the R2 storage-seam design with its five ffmpeg call sites, six numbered risks with acceptance criteria (volume ownership vs the non-root user, port contract, `X-Forwarded-For` hop count, two-processes-one-SQLite, builder limits, ffmpeg in the image), CI/CD workflows, and a 7-phase rollout.
