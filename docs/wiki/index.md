...
- [[canvas-mode]] - **Proposed ADR** (2026-07-29): Canvas Mode — Freepik/Flora-style node-graph canvas (`/canvas`) for chained generations (character → image → remix → video → upscale) OVER the unchanged generation lifecycle. Canvas aggregate CITES generations (film precedent); one contract delta (`inputGenerationId`); upscale/remove-bg as catalog pseudo-models; engine `@xyflow/react`. Spec: `docs/superpowers/specs/2026-07-29-canvas-mode-design.md`.
